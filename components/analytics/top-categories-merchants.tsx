...
function RankedList({ title, empty, children, maxValue }: { title: string; empty: string; children: ReactNode; maxValue: number }) {
  return (
    <div>
      <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">{title}</h3>
      {maxValue === 0 ? <p className="text-sm text-zinc-500 dark:text-zinc-400">{empty}</p> : <ul className="space-y-2">{children}</ul>}
    </div>
  );
}
