        ...
        {pageItems.map((tx) => (
          <li key={tx.id} className="flex justify-between border-b border-zinc-100 pb-2 dark:border-zinc-800">
            <span className="truncate max-w-xs text-zinc-700 dark:text-zinc-300">
              ...
              {tx.categories?.name && (
                <span className="ml-1.5 text-xs text-zinc-500 dark:text-zinc-400">· {resolveCategoryName(tx.categories, t)}</span>
              )}
              <span className="ml-1.5 text-xs text-zinc-500 dark:text-zinc-400">· {formatDate(tx.date)}</span>
              {(tx.kind === "transfer_debit" || tx.kind === "transfer_credit") && (
                <span className="ml-1.5 text-xs text-zinc-500 dark:text-zinc-400">· {t("dashboard.recentTransactions.transfer")}</span>
              )}
            </span>
            <span className={`ml-4 shrink-0 font-medium ${tx.amount_cents < 0 ? "text-red-600 dark:text-red-400" : "text-green-700 dark:text-green-500"}`}>
              {tx.amount_cents < 0 ? "−" : "+"}
              {formatEuros(Math.abs(tx.amount_cents))}
            </span>
          </li>
        ))}
        {transactions.length === 0 && <li className="text-zinc-500 dark:text-zinc-400">{t("dashboard.recentTransactions.empty")}</li>}
      </ul>
      <Pagination
        page={page}
