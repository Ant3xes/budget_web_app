...
export function FixedChargesSummary({ charges, totalCents }: FixedChargesSummaryProps) {
  ...
  return (
    <DashboardCard>
      ...
      {charges.length === 0 ? (
        <p className="text-sm text-zinc-500 dark:text-zinc-400">{t("dashboard.fixedCharges.empty")}</p>
      ) : (
        <>
              ...
              return (
                <li key={charge.id} className="flex items-center justify-between border-b border-zinc-100 pb-2 last:border-0 last:pb-0 dark:border-zinc-800">
                  <span className="truncate text-zinc-700 dark:text-zinc-300">
                    ...
                    {charge.name}
                    <span className={`ml-1.5 text-xs ${dueSoon ? "font-semibold text-red-600 dark:text-red-400" : "text-zinc-500 dark:text-zinc-400"}`}>
