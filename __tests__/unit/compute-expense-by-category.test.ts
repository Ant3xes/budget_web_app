...
describe("computeExpenseByCategory", () => {
  ...
  it("aggregates all expenses by category when no bounds", () => {
    ...
    expect(result).toEqual([
      { name: "Alimentation", value: 8000, color: "#22c55e", categoryId: null },
      { name: "Logement", value: 8000, color: "#3b82f6", categoryId: null },
      { name: "Sans catégorie", value: 2000, color: "#94a3b8", categoryId: null },
    ]);
  });

  it("filters by inclusive date window", () => {
    const result = computeExpenseByCategory(txs, "2026-01-01", "2026-01-31");
    expect(result).toEqual([{ name: "Alimentation", value: 8000, color: "#22c55e", categoryId: null }]);
  });
  ...
  it("carries the category id through for drill-down links", () => {
    ...
    expect(result).toEqual([{ name: "Alimentation", value: 5000, color: "#22c55e", categoryId: "cat-1" }]);
  });
});
